...
const Ofertas = () => {
  ...
  const addToOffersCart = (item) => {
    setCart((prev) => {
      ...
      if (idx > -1) {
        ...
        next = [
          ...prev,
          { id: item.id, nombre: item.nombre, precio: item.precioOferta, qty: 1 },
        ];
      }
      setToast({ title: "Agregado", text: `${item.nombre} • ${currency(item.precioOferta)}` });
  ...
  const emptyCart = () => {
    setCart([]);
    setMiniOpen(false);
    setToast({ title: "Carrito vacío", text: "Se vació el carrito de ofertas" });
  ...
  const handleSimulatePayment = () => {
    if (cart.length === 0) {
      setToast({ title: "Carrito vacío", text: "Agregá algún producto antes de pagar." });
  ...
  const confirmPayment = () => {
    // Aquí podrías ejecutar lógica real (API, limpiar carrito, etc.)
    ...
    <main className="ofertas-container">
      <header className="ofertas-header">
        <div>
          ...
          <p className="ofertas-sub">Descuentos por tiempo limitado. Entregas y calidad gourmet.</p>
        </div>
        ...
        <div className="offers-mini-toggle">
          ...
          >
            🛍️ Ofertas <span className="mini-count">{cart.reduce((s,i)=>s+i.qty,0)}</span>
          </button>
        </div>
      </header>
...
<img
  ...
  loading="lazy"
  onError={(e) => {
    // evita bucle si el placeholder falla
    if (e.target.dataset.fallback) return;
    e.target.dataset.fallback = "true";
    // placeholder SVG muy liviano (gris con icono de cuchillo/tenedor)
    e.target.src =
...
/>
            ...
            <div className="oferta-precios">
              <span className="precio-original">{currency(item.precioOriginal)}</span>
            ...
            <div className="oferta-acciones">
              ...
              <Link to={`/producto/${item.id}`} className="btn-detalle">
                Ver detalle
              </Link>
            </div>
          </article>
      ...
      {/* MINI CART (resumido o expandible) */}
      <aside
        id="offers-mini-cart"
      ...
      >
        <div className="mini-header">
          ...
          <div className="mini-actions">
            <button className="mini-empty" onClick={emptyCart} aria-label="Vaciar carrito">Vaciar</button>
            <button className="mini-close" onClick={() => setMiniOpen(false)} aria-label="Cerrar">✕</button>
          </div>
        </div>

        <div className="mini-body">
          {cart.length === 0 ? (
            <div className="mini-empty-state">No hay productos. Agregá alguna oferta.</div>
          ) : (
            <>
                  ...
                  <li key={p.id} className="mini-item">
                    <div className="mini-left">
                      ...
                      <div className="mini-qty">
                        <button onClick={() => changeQty(p.id, -1)} aria-label="Disminuir cantidad">−</button>
                        <span>{p.qty}</span>
                        <button onClick={() => changeQty(p.id, +1)} aria-label="Aumentar cantidad">+</button>
                      </div>
                    </div>
                    <div className="mini-right">
                      <div className="mini-sub">{currency(p.precio * p.qty)}</div>
                      <button className="mini-remove" onClick={() => removeItem(p.id)} aria-label="Eliminar">Eliminar</button>
                    </div>
                  </li>
                ))}
              </ul>
              ...
              <div className="mini-cta">
                <button className="btn-pay" onClick={handleSimulatePayment}>Pagar (simulado)</button>
              </div>
            </>
          )}
        </div>
        ...
        <div className="pay-modal" role="dialog" aria-modal="true">
          <div className="pay-card">
            <header className="pay-card-header">
              ...
              <button className="pay-close" onClick={() => setPayModalOpen(false)} aria-label="Cerrar">✕</button>
            </header>

            <div className="pay-card-body">
              <p className="pay-intro">
                Estás por pagar <strong>{currency(total)}</strong>. Este es un pago simulado.
              </p>
            ...
            <footer className="pay-card-footer">
              <button className="btn-secondary" onClick={() => setPayModalOpen(false)}>Cancelar</button>
